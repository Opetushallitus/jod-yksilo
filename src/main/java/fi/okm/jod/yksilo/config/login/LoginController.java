...
import static fi.okm.jod.yksilo.config.login.LoginConfig.MPASSID_REGISTRATION_ID;
...
class LoginController {
  ...
  private final MpassidProperties mpassidProperties;
  ...
  public static final String MPASSID_REDIRECT_URI =
      "/oauth2/authorization/" + MPASSID_REGISTRATION_ID;

  LoginController(MpassidProperties mpassidProperties) {
    this.mpassidProperties = mpassidProperties;
  }
  ...
  void login(
    ...
    if (mpassidProperties.isEnabled() && MPASSID_REGISTRATION_ID.equals(method)) {
      response.sendRedirect(request.getContextPath() + MPASSID_REDIRECT_URI);
    } else {
