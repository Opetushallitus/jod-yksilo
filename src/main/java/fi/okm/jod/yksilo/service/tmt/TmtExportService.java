...
public class TmtExportService {
  ...
  TmtExportService(
      ...
      RestClient.Builder tmtExportRestClientBuilder,
    ...
    this.restClient =
        tmtExportRestClientBuilder
            .baseUrl(tmtConfiguration.getApiUrl())
            .defaultHeader("KIPA-Subscription-Key", tmtConfiguration.getKipaSubscriptionKey())
            .build();
  }
  ...
  public void export(JodUser jodUser, AccessToken token) {
    ...
    try {
      ...
      transactionTemplate.execute(status -> tapahtumat.saveAndFlush(tapahtuma));
    }
  }
