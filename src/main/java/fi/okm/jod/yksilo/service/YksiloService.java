...
public class YksiloService {
  ...
  public YksiloDto findYksilo(JodUser user, CsrfToken csrfToken) {
    return yksilot
        .findById(user.getId())
        .map(yksilo -> Mapper.mapYksilo(yksilo, csrfToken))
        .orElseThrow(() -> new NotFoundException("Yksilöä ei löytynyt"));
  }
}
